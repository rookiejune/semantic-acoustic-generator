...
FACTOR_CODEBOOKS="${FACTOR_CODEBOOKS:-2}"
FACTOR_SAMPLE_LIMIT="${FACTOR_SAMPLE_LIMIT:-128}"
FACTOR_BATCH_SIZE="${FACTOR_BATCH_SIZE:-16}"
FACTOR_MAX_STEPS="${FACTOR_MAX_STEPS:-2000}"
FACTOR_OUTPUT_SUBDIR="${FACTOR_OUTPUT_SUBDIR:-013/factor-${FACTOR_CODEBOOKS}cb-${FACTOR_SAMPLE_LIMIT}-${FACTOR_MAX_STEPS}}"

echo "{\"event\":\"job.launch\",\"project\":\"semantic-acoustic-generator\",\"experiment\":\"013\",\"stage\":\"factor-${FACTOR_CODEBOOKS}cb\"}"
"${SEMANTIC_ACOUSTIC_GENERATOR_PYTHON}" scripts/train.py \
    experiment=013_longcat_factor_scale \
    model.feature_codebooks="${FACTOR_CODEBOOKS}" \
