...
from semantic_acoustic_codec.model import RectifiedFlowRuntime, build_route, teacher_features
...
def main() -> None:
    ...
    _route_smoke(teacher, decoder, routes=_routes(args.routes))
    _artifact_smoke(teacher, decoder)
    ...
    *,
    ...
    for route in routes:
        if route is Route.RVQ and importlib.util.find_spec("transformers") is None:
            ...
            continue
        modules = build_route(route, teacher, condition_dim=12, decoder=decoder)
...
def _artifact_smoke(teacher: LongCatTeacher, decoder: DecoderConfig) -> None:
    semantic, _, mask = _batch()
    config = SemanticCodecConfig(route=Route.FM, condition_dim=12, decoder=decoder)
    ...
    waveform = codec.decode(semantic, mask=mask, generator=torch.Generator().manual_seed(1))
    if features.shape != (*semantic.shape[:2], teacher.acoustic_feature_dim):
    ...
    with tempfile.TemporaryDirectory(prefix="semantic-acoustic-codec-") as tmp:
        save_artifact(tmp, codec, config)
        loaded = load_artifact(tmp, teacher=teacher)
