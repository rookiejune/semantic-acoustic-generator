...
class AcousticRVQMTPDecoder(nn.Module):
    """Qwen3-Omni-style RVQ predictor with temporal AR plus intra-frame MTP.

    The temporal backbone models frame-to-frame dependency through the first
    acoustic codebook. A lightweight MTP predictor then fills the remaining
    residual codebooks inside each frame.
    """
    ...
    def forward(
        ...
        *,
        ...
        if target_acoustic_codes is not None:
            _validate_targets(target_acoustic_codes, condition, self.codebooks, self.codebook_sizes, frame_mask)
            previous_first = _shift_right(target_acoustic_codes[..., 0], frame_mask)
        else:
    ...
    def generate(
        ...
        *,
        ...
        for frame in range(condition.size(1)):
            decoder_input = (
                condition_hidden[:, frame]
                + self.first_bos
                + self._embedding(0, previous_first)
            )
            ...
            past_key_values = state_output.past_key_values
            if past_key_values is None:
                raise RuntimeError("RVQ MTP temporal decoder did not return a generation cache.")
    ...
    def _sample_mtp(
        ...
        *,
        ...
        for codebook in range(1, self.codebooks):
            ...
            past_key_values = state_output.past_key_values
            if past_key_values is None:
                raise RuntimeError("RVQ MTP predictor did not return a generation cache.")
            state = state_output.last_hidden_state[:, -1]
