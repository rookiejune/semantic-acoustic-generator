"""Strict configuration boundary for semantic-to-acoustic generator training."""
...
def resolve_output_dir(config: TrainConfig) -> Path:
    ...
    root = os.environ.get("SEMANTIC_ACOUSTIC_GENERATOR_TRAIN_ROOT")
    if root is None:
        dynamic = os.environ.get("DYNAMIC_HOME")
        root = (
            "/tmp/semantic-acoustic-generator"
            if dynamic is None
            else f"{dynamic}/train/semantic-acoustic-generator"
        )
