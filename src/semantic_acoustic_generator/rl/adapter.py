...
from .types import GeneratorCandidate, GeneratorRewardBatch, GeneratorRollout

if TYPE_CHECKING:
    from semantic_acoustic_generator.runtime import GeneratorRuntime


class GeneratorRLAdapter:
    """Generator-owned adapter from semantic/acoustic candidates to anytrain RL tensors.

    The adapter owns generation rollout records and reward plumbing, while anytrain
    continues to own only tensor-level RL objectives.
    """

    def __init__(self, runtime: GeneratorRuntime | None = None) -> None:
    ...
    def rollout(
        ...
        *,
        ...
        if self.runtime is None:
            raise ValueError("runtime is required for generator rollout.")
        if group_size <= 0:
    ...
    def score(
        ...
        *,
        ...
        if rewards is None:
            raise ValueError("generator reward scoring is task-specific; pass rewards explicitly.")
    ...
    def to_grpo_batch(
        self,
        reward_batch: GeneratorRewardBatch,
        *,
...
def _reference_features(runtime: GeneratorRuntime, batch: GeneratorBatch) -> Tensor | None:
...
def _metadata(batch: GeneratorBatch, sample_id: int) -> Mapping[str, object]:
    if not batch.metadata:
        return {}
    return {"pair": batch.metadata[sample_id]}


def _validate_reward_shape(rewards: Tensor, candidates: GeneratorRollout) -> None:
