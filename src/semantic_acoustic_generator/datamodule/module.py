...
class DataModule(pl.LightningDataModule):
    ...
    def val_dataloader(
        self,
    ) -> DataLoader[GeneratorBatch] | list[DataLoader[GeneratorBatch]]:
...
def single_batch_loader(
    value: GeneratorBatch,
) -> DataLoader[GeneratorBatch]:
    dataset = cast(Dataset[GeneratorBatch], cast(object, [value]))
    loader = DataLoader(
        dataset,
        batch_size=1,
        num_workers=0,
    ...
    return cast(DataLoader[GeneratorBatch], cast(object, loader))


def _single_batch(values: Sequence[GeneratorBatch]) -> GeneratorBatch:
    if len(values) != 1:
