...
class LongCatFirstCodebookAdapter(nn.Module):
    """Expose LongCat stage-0 quantized embeddings as 16-D acoustic features."""

    def __init__(self, backend: SemanticAcousticCodec) -> None:
        super().__init__()
        if backend.name != "longcat":
            raise ValueError("LongCat first-codebook features require backend.name='longcat'.")
        if backend.acoustic_layout is not AcousticLayout.FRAME_ALIGNED:
            raise ValueError("LongCat first-codebook features require frame-aligned units.")
        self.backend = backend
        stage = self._stage()
        size_a = _positive_int(stage.codebook_size_a, "codebook_size_a")
        size_b = _positive_int(stage.codebook_size_b, "codebook_size_b")
        dim_a = _embedding_dim(stage.codebook_a, "codebook_a")
        dim_b = _embedding_dim(stage.codebook_b, "codebook_b")
        if size_a != stage.codebook_a.num_embeddings:
            raise ValueError("LongCat stage-0 codebook_size_a must match codebook_a.")
        if size_b != stage.codebook_b.num_embeddings:
            raise ValueError("LongCat stage-0 codebook_size_b must match codebook_b.")
        if not backend.acoustic_codebook_sizes:
            raise ValueError("LongCat backend must expose at least one acoustic codebook.")
        if backend.acoustic_codebook_sizes[0] != size_a * size_b:
            raise ValueError("LongCat stage-0 factor codebooks must match the composite codebook size.")
        ...
        if projected_dim != backend.acoustic_feature_dim:
            raise ValueError(
                "LongCat stage-0 projections must produce the decoder acoustic feature dimension."
            )
        self._factor_sizes = (size_a, size_b)
        self._factor_dims = (dim_a, dim_b)
    ...
    def acoustic_feature_dim(self) -> int:
        return sum(self._factor_dims)

    @property
    def factor_codebooks(self) -> tuple[Tensor, Tensor]:
        stage = self._stage()
        return stage.codebook_a.weight, stage.codebook_b.weight
    ...
    def acoustic_codes_to_features(self, acoustic_codes: Tensor) -> Tensor:
        ...
        weight_a, weight_b = self.factor_codebooks
        features_a = F.embedding(factors[..., 0].to(device=weight_a.device), weight_a)
        features_b = F.embedding(factors[..., 1].to(device=weight_b.device), weight_b)
        return torch.cat((features_a, features_b), dim=-1)
    ...
    def factor_codes(self, acoustic_codes: Tensor) -> Tensor:
        ...
        size_a, size_b = self._factor_sizes
        composite = acoustic_codes[..., 0]
        if bool(((composite < 0) | (composite >= size_a * size_b)).any()):
            raise ValueError("LongCat stage-0 codes contain an ID outside the codebook.")
        codes_a = torch.div(composite, size_b, rounding_mode="floor")
        codes_b = composite.remainder(size_b)
        return torch.stack((codes_a, codes_b), dim=-1)

    @torch.no_grad()
    def snap_features(self, acoustic_features: Tensor) -> Tensor:
        factors = self.features_to_factor_codes(acoustic_features)
        codebook_a, codebook_b = self.factor_codebooks
        return torch.cat(
            (
                F.embedding(factors[..., 0], codebook_a),
                F.embedding(factors[..., 1], codebook_b),
            ),
            dim=-1,
        )
    ...
    def features_to_factor_codes(self, acoustic_features: Tensor) -> Tensor:
        ...
        dim_a, dim_b = self._factor_dims
        factors = acoustic_features.split((dim_a, dim_b), dim=-1)
        indices = []
    ...
    def project_features(self, acoustic_features: Tensor) -> Tensor:
        ...
        stage = self._stage()
        dim_a, dim_b = self._factor_dims
        reference = stage.codebook_a.weight
        features = acoustic_features.to(device=reference.device, dtype=reference.dtype)
        features_a = features[..., :dim_a].transpose(1, 2).contiguous()
        features_b = features[..., dim_a : dim_a + dim_b].transpose(1, 2).contiguous()
        return torch.cat(
    ...
    def native_stage0_features(self, acoustic_codes: Tensor) -> Tensor:
        ...
        output = convert(acoustic_codes[..., :1].transpose(1, 2).contiguous())
        ...
        features = output[0]
        if features.dim() != 3:
            raise ValueError("LongCat native stage-0 features must have shape [batch, dim, time].")
    ...
    def _stage(self) -> Any:
        ...
        if quantizers is None or len(quantizers) < 1:
            raise TypeError("LongCat decoder must expose at least one acoustic quantizer stage.")
        stage = quantizers[0]
        for name in (
            "codebook_size_a",
            "codebook_size_b",
            ...
            if not hasattr(stage, name):
                raise TypeError(f"LongCat stage-0 quantizer must expose {name}.")
        return stage
...
def adapt_backend(
    ...
    if adapter is FeatureAdapter.NONE:
        return backend
    if adapter is FeatureAdapter.LONGCAT_FIRST_CODEBOOK:
        if isinstance(backend, LongCatFirstCodebookAdapter):
            return backend
        return cast(SemanticAcousticCodec, LongCatFirstCodebookAdapter(backend))
    raise AssertionError(f"unsupported feature adapter: {adapter}")


def _positive_int(value: object, name: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        raise TypeError(f"LongCat stage-0 {name} must be a positive integer.")
    return value


def _embedding_dim(value: object, name: str) -> int:
    if not isinstance(value, nn.Embedding):
        raise TypeError(f"LongCat stage-0 {name} must be an nn.Embedding.")
...
__all__ = ["LongCatFirstCodebookAdapter", "adapt_backend"]
